...
// No leading/trailing blank lines of its own — prependEntry() controls all
// spacing around it, since the right amount differs depending on whether
// it's landing right after frontmatter or right after a Watchlist section.
...
function prependEntry(raw, entry) {
  ...
  // Normalize whitespace at the seam rather than assuming a fixed amount —
  // insertionPoint === 0 (nothing before it but frontmatter) wants exactly
  // one leading newline; landing after a Watchlist section (which already
  // ends in a blank line) wants exactly one blank line (two newlines).
  const before = body.slice(0, insertionPoint).replace(/\n*$/, insertionPoint === 0 ? '\n' : '\n\n');
  const after = body.slice(insertionPoint).replace(/^\n+/, '');
  const newBody = `${before}${formatEntryBlock(entry)}\n\n${after}`;

  const frontmatterLines = Object.entries(frontmatter).map(([key, value]) => `${key}: ${value}`);
  return `---\n${frontmatterLines.join('\n')}\n---\n${newBody}`;
}

// Reassembles a raw file string from parsed frontmatter + body, undoing
// splitFrontmatter(). Only meant for our own generated content — assumes
// the simple "key: value" frontmatter format this whole module works with.
function reassemble(frontmatter, body) {
  const frontmatterLines = Object.entries(frontmatter).map(([key, value]) => `${key}: ${value}`);
  return `---\n${frontmatterLines.join('\n')}\n---\n${body}`;
}

// Replaces any existing "## Watchlist" section with a freshly generated one
// (or removes it entirely if `watchlist` is empty), positioned right after
// frontmatter, before any dated entries. Unlike entries — which accumulate,
// newest on top — the watchlist reflects "what's worth watching right now"
// and is fully replaced each run rather than appended to; call this BEFORE
// prependEntry() so the new entry lands after the fresh watchlist.
function replaceWatchlist(raw, watchlist) {
  const { frontmatter, body } = splitFrontmatter(raw);
  ...
  if (headingMatch) {
    ...
    const nextHeadingMatch = rest.match(/^##\s+/m);
    const sectionEnd = nextHeadingMatch ? afterHeading + nextHeadingMatch.index : body.length;
    remaining = body.slice(0, sectionStart) + body.slice(sectionEnd);
  }
  remaining = remaining.replace(/^\n+/, '');

  if (!watchlist.length) {
    return reassemble(frontmatter, `\n${remaining}`);
  }

  const watchlistLines = ['## Watchlist', ...watchlist.map((w) => `- $${w.ticker}: ${w.note}`)];
  return reassemble(frontmatter, `\n${watchlistLines.join('\n')}\n\n${remaining}`);
}
